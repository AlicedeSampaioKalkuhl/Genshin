...
```{r character stats}
character_builds <- merge(builds,characters,on="character") %>% 
merge(weapons, on="weapon") %>% 
mutate(ATK = ((level/90)*ATK_base)*(1+ATK_multiplier+(level/90)*ATKmultiplier_base)+ATK_flat,
HP = ((lv/90)*HP_base)*(1+HP_multiplier+(lv/90)*HPmultiplier_base)+HP_flat,
DEF = ((lv/90)*DEF_base)*(1+DEF_multiplier+(lv/90)*DEFmultiplier_base)+DEF_flat,
ER = (lv/90)*ER_base+ER_artifacts,
EM = (lv/90)*EM_base+EM_artifacts,
CritRate = (lv/90)*CritRate_base+CritRate_artifacts,
CritDMG = (lv/90)*CritDMG_base+CritDMG_artifacts,
HealingBonus = (lv/90)*HealingBonus_base+HealingBonus_artifacts,
AnemoDMG = (lv/90)*AnemoDMG_base+AnemoDMG_artifacts,
CryoDMG = (lv/90)*CryoDMG_base,
DendroDMG = (lv/90)*DendroDMG_base,
ElectroDMG = (lv/90)*ElectroDMG_base,
GeoDMG = (lv/90)*GeoDMG_base,
PhysicalDMG = (lv/90)*PhysicalDMG_base,
PyroDMG = (lv/90)*PyroDMG_base) %>% 
